...
(async function renderComplaintsByFaculty() {
    ...
    try {
        const [complaints, faculties, departments] = await Promise.all([
            fetch(`${base}../data/complaints.json`).then((r) => r.json()),
            fetch(`${base}../data/faculties.json`).then((r) => r.json()),
            fetch(`${base}../data/departments.json`).then((r) => r.json()),
        ]);
            ...
            recentTbody.innerHTML = latest
                .map((c) => {
                    ...
                    <tr>
                        ...
                        <td>${date}</td>
                        <td>
                            <a href="complaint/view.html?id=${encodeURIComponent(
                                c.id
